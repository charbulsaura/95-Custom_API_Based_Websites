...
def api_call_plot_graph(symbol, entry_count, date_range):
    ...
    print(f"date_range before graph plot: {date_range}")
    ...
    if len(stock_list) > date_range:
        ...
        low_y = low_y[len(stock_list) - date_range:]
        open_y = open_y[len(stock_list) - date_range:]
        close_y = close_y[len(stock_list) - date_range:]
    ...
    if stonks_reload == False:
        ...
        fig.savefig(f'static/img/stonks_{SYMBOLS}_date_{date_range}_{entry_count}.png', bbox_inches='tight')
        filename = f'img/stonks_{SYMBOLS}_date_{date_range}_{entry_count}.png'
        # stonks_reload = False
...
def home():
    global monthly_average, monthly_average_low, SYMBOLS, date_x, filename, date_range, entry_count
    # cache.init_app(app)
    # cache.clear()
